...
class GroupController extends Controller
{
    public function index(Request $request) {
        ...
        $count = Group::count();
        $data = Group::where($field, 'LIKE' , '%'.$filter.'%')->take($limit)->skip($offset)->orderBy($field, $order)->get();
    ...
    public function update(GroupRequest $request, $id){
        ...
        Group::where('groupId','=',$id)->update($data);
    ...
    public function checkValue(Request $request){
        ...
        if($id == 0){
            $count = Group::where('groupName','=',$value)->count();
        ...
        //Else if the item is to be updated
            ...
            $count = Group::where('groupName','=',$value)->where('groupId','=',$id)->count();
            if($count>0){
                $status = 200;
                $message = 'Data is available';
            }
                ...
                $count2 = Group::where('groupName','=',$value)->count();
